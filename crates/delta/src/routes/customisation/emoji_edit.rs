...
pub async fn edit_emoji(
    ...
    if data.name.is_none() && data.metadata.is_none() {
...
mod test {
    ...
    #[rocket::async_test]
    async fn edit_emoji_name_and_metadata_as_creator() {
        ...
        let response = harness
            ...
            .body(
                json!(v0::DataEditEmoji {
                    ...
                    metadata: Some(v0::EmojiMetadata {
                        width: Some(128),
                        height: Some(128),
                        file_size: Some(4096),
                    }),
        ...
        assert_eq!(edited.metadata.as_ref().and_then(|v| v.width), Some(128));
        assert_eq!(edited.metadata.as_ref().and_then(|v| v.height), Some(128));
        assert_eq!(
            edited.metadata.expect("metadata").file_size,
            Some(4096)
        );
    }
